    ...
    async void Start()
    {
        ...
        websocket.OnMessage += (bytes) =>
        {
           ...
           // Debug.Log("OnMessage! " + message);
            ...
            {
                case turretModelId:
                moveObject(turret, GetValue(message));
                    break;
                case shoulderModelId:
                    moveObject(shoulder, GetValue(message));
                    break;
                case elbowModelId:
                    moveObject(elbow, GetValue(message));
                    break;
                case wristModelId:
                    moveObject(wrist, GetValue(message));
                    break;
                case clampModelId:
                    moveObject(clampBottom, GetValue(message));
                    moveObject(clampTop, GetValue(message));
                    break;
                case clampTurretModelId:
                    moveObject(clampTurret, GetValue(message));
                    break;
                case backPaltModelId:
                    moveObject(backPal, GetValue(message));
                    moveObject(backPal2, GetValue(message));

                    break;
                case frontPalModelId:
                    moveObject(frontPal, GetValue(message));
                    moveObject(frontPal2, GetValue(message));
